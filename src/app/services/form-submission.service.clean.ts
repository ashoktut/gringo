...
export class FormSubmissionService {
    ...
    if (formType === 'RFQ') {
      ...
      this.sendRfqEmail(submission.submissionId, formData, formStructure).subscribe({
        next: (emailResult) => {
          ...
          this.updateSubmissionInternally(submission);
        },
        error: (error) => {
  ...
  private sendRfqEmail(
    ...
    console.log('📧 Sending RFQ email for submission:', submissionId);

    const recipients = this.extractCcMailAddresses(formData);
    const clientEmail = formData.clientEmail || '';

    if (!clientEmail && recipients.length === 0) {
    ...
    return this.emailService.sendRfqEmail(submissionId, formData, formStructure).pipe(
      map(result => ({
        status: 'sent' as const,
        sentAt: new Date(),
        recipients: [...recipients, clientEmail].filter(Boolean)
      })),
      catchError(error => {
  ...
  private processRfqWithDocx(formData: any): Observable<any> {
    ...
    return this.templateService.getTemplatesForForm('rfq').pipe(
      switchMap(templates => {
        ...
        console.log('🔧 Processing RFQ with template:', template.name);
        console.log('📮 Email recipients:', { clientEmail, recipients });
  ...
  // Update existing submission (PUBLIC METHOD)
  updateSubmission(submissionId: string, formData: any, formStructure: FormSection[]): Observable<FormSubmission> {
    const index = this.submissions.findIndex(s => s.submissionId === submissionId);
    if (index === -1) {
      return throwError(() => new Error(`Submission with ID ${submissionId} not found`));
    }

    const existingSubmission = this.submissions[index];
    ...
    return this.indexedDbService.save(this.indexedDbService.STORES.SUBMISSIONS, submissionId, sanitizedSubmission).pipe(
      tap(() => {
        this.submissionsSubject.next([...this.submissions]);
        console.log('✅ Submission updated in IndexedDB:', submissionId);
      }),
      map(() => updatedSubmission),
      catchError(error => {
  ...
  /**
   * Update existing submission internally (for internal use only)
   */
  private updateSubmissionInternally(updatedSubmission: FormSubmission): void {
  ...
  /**
   * Fallback method for sanitizing complex objects
   */
  ...
  private loadSubmissionsFromStorage(): void {
    this.indexedDbService.getAll(this.indexedDbService.STORES.SUBMISSIONS).subscribe({
      next: (data) => {
        if (data && data.length > 0) {
          this.submissions = data.map(item => item.data as FormSubmission);
          this.submissionsSubject.next([...this.submissions]);
          console.log('✅ Submissions loaded from IndexedDB:', this.submissions.length);
        } else {
          console.log('ℹ️ No submissions found in IndexedDB');
        }
      },
      error: (error) => {
